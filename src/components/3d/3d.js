  ...
  var o3dSign = (function () {
    ...
    if (signEffect.all) {
      ...
      setInterval(function () {
        var index = getIndex(4);
        transform( targets[keyMap[index]], 2000 );
      }, 10000);
    }
    ...
    $(document).keydown(function (event) {
        //空格键控制
    if (event.keyCode == 32) {
        transform(targets[keyMap[getIndex(4)]], 2000)　　　　
    }　　
    });
    ...
    var keyMap = ['table', 'sphere', 'helix', 'grid'];
    var table = [];
    var targetIndex = 0;
    ...
    function transform(a, e) {
      ...
      for (var c = 0; c < objects.length; c++) {
        ...
        new TWEEN.Tween(b.position).to({
          ...
          y: d.position.y,
          z: d.position.z
        }, Math.random() * e + e).easing(TWEEN.Easing.Exponential.InOut).start();
        new TWEEN.Tween(b.rotation).to({
          x: d.rotation.x,
          y: d.rotation.y,
          z: d.rotation.z
        }, Math.random() * e + e).easing(TWEEN.Easing.Exponential.InOut).start()
    ...
    function render() {
      ...
      if (rotate) {
        camera.position.x = Math.cos(a) * 1500;
    ...
    function resizeImg(f, d, e) {
      var srcWidth = f.width;
      var srcHeight = f.height;
      var c = 1;
      ...
      if (b < d) {
        f.style.paddingLeft = ((d - b) / 2).toString() + "px"
      }
      if (a < e) {
        f.style.paddingTop = ((e - a) / 2).toString() + "px"
      }
    }
    ...
    function getImgs() {
      ...
      for (var d = 0; d < arrfiles.length; d++) {
        ...
        var b = c[0];
        if (c[0].endsWith("/132")) {
          b = c[0].substring(0, c[0].lastIndexOf("/")) + "/0"
        }
        c[3] = b;
